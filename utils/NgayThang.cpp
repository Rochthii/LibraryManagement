...
int tinhSoNgayQuaHan(const std::string &ngayMuon, std::ostream &out){
    ...
    if (tMuon == -1)
        return -1; // Loi chuyen doi ngay muon

    double diff_seconds = difftime(now, tMuon);       // tinh so giay chenh lech giua hien tai va ngay muon
    double diff_days = diff_seconds / (60 * 60 * 24); // chuyen so giay sang so ngay (chia cho 60*60*24)
    int soNgayQuaHan = static_cast<int>(diff_days) - SO_NGAY_MUON_TOI_DA;     // tinh so ngay da qua han (so ngay muon - so ngay toi da)
    return soNgayQuaHan > 0 ? soNgayQuaHan : 0;       // neu so ngay qua han < 0 thi tra ve 0 (chua qua han)
}

// phan tich chuoi ngay "dd/mm/yyyy" thanh cac so nguyen
